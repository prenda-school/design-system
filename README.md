...
Install packages in root dir using `nvm use && npm i` then run `npm run storybook` in one terminal and `npm run start in another terminal`. Navigate to `localhost:6006`. If a component you are working on doesn't have a story then you can add one in the `/stories/`.
